...
char *testName(int test) {
  ...
  if (test == Sum_getNNZ) return "Sum_getNNZ";
  if (test == Diff_getNNZ) return "Diff_getNNZ";
  if (test == ScalarMult_getNNZ) return "ScalarMult_getNNZ";
  if (test == Product_getNNZ) return "Product_getNNZ";
...
uint8_t runTest(Matrix *pA, Matrix *pB, Matrix *pC, Matrix *pD, int test) {
  ...
  switch(test) {
    ...
    case Copy_getNNZ:
      {
        *pC = copy(A);
        if (NNZ(*pC) != 0) return 1;
        changeEntry(A, 1, 1, 1);
        if (NNZ(*pC) != 0) return 2;
        *pD = copy(A);
        if (NNZ(*pD) != 1) return 3;
        return 0;
      }
    ...
    case ScalarMult_getNNZ:
      {
        ...
        changeEntry(A, 3, 3, 0);
        *pC = scalarMult(-20, A);
        if (NNZ(*pC) != 4) return 1;
        makeZero(A);
        *pD = scalarMult(-20, A);
        if (NNZ(*pD) != 0) return 2;
        return 0;
      }
    case Product_getNNZ:
      {
    ...
    case NonEmpty_equals:
      {
        ...
        *pD = diff(As2, A);
        freeMatrix(&As1);
        freeMatrix(&As2);
        if (!equals(*pC, *pD)) return 6;

        freeMatrix(pC);
        *pC = product(A, B);
        if (!equals(*pC, A)) return 7;
